...
function check_dob(field_name, message) {
  if (form.elements[field_name] && (form.elements[field_name].type != "hidden")) {
    var field_value = form.elements[field_name].value;

    if (field_value != '' && !field_value.match(/^[0-9]{4}\/[0-9]{2}\/[0-9]{2}$/)) {
      error_message = error_message + "* " + message + "\n";
      error = true;
    }
  }
...
function check_phone(field_name, message) {
  if (form.elements[field_name] && (form.elements[field_name].type != "hidden")) {
    ...
    if (field_value != '' && !field_value.match(/^[0-9\-]+$/)) {
      error_message = error_message + "* " + message + "\n";
      error = true;
    }
  }
...
function check_hirakana(field_name, message) {
  if (form.elements[field_name] && (form.elements[field_name].type != "hidden")) {
    var field_value = form.elements[field_name].value;

    if (field_value != '' && !field_value.match(<?php echo ENTRY_HIRAKANA_REGEXP_JS; ?>)) {
      error_message = error_message + "* " + message + "\n";
      error = true;
    }
  }
...
<?php if (DISPLAY_PRIVACY_CONDITIONS == 'true' && !$_SESSION['visitors_id']) echo '  if (form.elements["privacy_conditions"].checked != true){ error = true; error_message = error_message + "* " + "'. ERROR_PRIVACY_STATEMENT_NOT_ACCEPTED . '" + "\n";}' . "\n"; ?>
...
<?php if (FURIKANA_NESESSARY) { ?>
  ...
  check_hirakana("firstname_kana", "<?php echo ENTRY_FIRST_NAME_KANA.ENTRY_HIRAKANA_NOMATCH; ?>");
  check_hirakana("lastname_kana",  "<?php echo ENTRY_LAST_NAME_KANA.ENTRY_HIRAKANA_NOMATCH; ?>");
<?php } ?>

<?php if (ACCOUNT_DOB == 'true' && (int)ENTRY_DOB_MIN_LENGTH != 0)
  echo '    check_dob  ("dob", "' . ENTRY_DATE_OF_BIRTH_ERROR . '");' . "\n";
?>
...
<?php } ?>
  ...
  check_phone("telephone", "<?php echo ENTRY_TELEPHONE_NOMATCH; ?>");
  check_phone("fax", "<?php echo ENTRY_FAX_NOMATCH; ?>");
