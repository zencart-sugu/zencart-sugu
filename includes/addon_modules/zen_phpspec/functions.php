<?php
function zen_spec_load_fixture($fixtures) {
  foreach ($fixtures as $fixture) {
    zen_spec_execute_sql_file('../../zen_phpspec/fixtures/' . $fixture . '.sql');
  }
}

function zen_spec_execute_sql_file($sql_file) {
...
//echo '*}'.$line.'<br>';
          ...
          case (substr($line_upper, 0, 11) == 'DROP TABLE ' && $param[2] != 'IF'):
            if (!$checkprivs = zen_spec_check_database_privs('DROP')) $result=sprintf(REASON_NO_PRIVILEGES,'DROP');
            if (!zen_spec_table_exists($param[2]) || zen_not_null($result)) {
              zen_spec_write_to_upgrade_exceptions_table($line, (zen_not_null($result) ? $result : sprintf(REASON_TABLE_DOESNT_EXIST,$param[2])), $sql_file);
              $ignore_line=true;
          ...
          case (substr($line_upper, 0, 13) == 'CREATE TABLE '):
            ...
            $result=zen_spec_table_exists($table);
            if ($result==true) {
              zen_spec_write_to_upgrade_exceptions_table($line, sprintf(REASON_TABLE_ALREADY_EXISTS,$table), $sql_file);
              $ignore_line=true;
          ...
          case (substr($line_upper, 0, 12) == 'INSERT INTO '):
            ...
            if (!$tbl_exists = zen_spec_table_exists($param[2])) $result=sprintf(REASON_TABLE_NOT_FOUND,$param[2]).' CHECK PREFIXES!';
            // check to see if INSERT command may be safely executed for "configuration" or "product_type_layout" tables
            if (($param[2]=='configuration'       && ($result=zen_spec_check_config_key($line))) or 
                ($param[2]=='product_type_layout' && ($result=zen_spec_check_product_type_layout_key($line))) or
                ($param[2]=='configuration_group' && ($result=zen_spec_check_cfggroup_key($line))) or
                (!$tbl_exists)    ) {
              zen_spec_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
          ...
          case (substr($line_upper, 0, 19) == 'INSERT IGNORE INTO '):
            //check to see if table prefix is going to match
            if (!$tbl_exists = zen_spec_table_exists($param[3])) {
          ...
          case (substr($line_upper, 0, 12) == 'ALTER TABLE '):
            ...
            if ($result=zen_spec_check_alter_command($param)) {
              zen_spec_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
          ...
          case (substr($line_upper, 0, 13) == 'RENAME TABLE '):
            ...
            if (zen_not_null(DB_PREFIX)) {
              zen_spec_write_to_upgrade_exceptions_table($line, 'RENAME TABLE command not supported by upgrader. Please use phpMyAdmin instead.', $sql_file);
              $ignore_line=true;
            }
            break;
          case (substr($line_upper, 0, 7) == 'UPDATE '):
            //check to see if table prefix is going to match
            if (!$tbl_exists = zen_spec_table_exists($param[1])) {
              zen_spec_write_to_upgrade_exceptions_table($line, sprintf(REASON_TABLE_NOT_FOUND,$param[1]).' CHECK PREFIXES!', $sql_file);
          ...
          case (substr($line_upper, 0, 14) == 'UPDATE IGNORE '):
            //check to see if table prefix is going to match
            if (!$tbl_exists = zen_spec_table_exists($param[2])) {
              zen_spec_write_to_upgrade_exceptions_table($line, sprintf(REASON_TABLE_NOT_FOUND,$param[2]).' CHECK PREFIXES!', $sql_file);
          ...
          case (substr($line_upper, 0, 11) == 'DROP INDEX '):
            ...
            if ($result=zen_spec_drop_index_command($param)) {
              zen_spec_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
          ...
          case (substr($line_upper, 0, 13) == 'CREATE INDEX ' || (strtoupper($param[0])=='CREATE' && strtoupper($param[2])=='INDEX')):
            ...
            if ($result=zen_spec_create_index_command($param)) {
              zen_spec_write_to_upgrade_exceptions_table($line, $result, $sql_file);
              $ignore_line=true;
              break;
            } else {
...
//        } // endif $table_prefix
        ...
        if ($complete_line) {
          ...
          if (zen_not_null($result) && !zen_spec_check_exceptions($result, $line) ) $errors[]=$result;
          // reset var's
          $newline = '';
          $keep_together=1;
          $complete_line = false;
          if ($ignore_line && !zen_spec_check_exceptions($result, $line)) $ignored_count++;
          $ignore_line=false;
...
function zen_spec_read_config_value($value) {
  $files_array = array();
	$string='';
...
function zen_spec_table_exists($tablename, $pre_install=false) {
  global $db, $db_test;
  if ($pre_install==true) {
...
function zen_spec_check_database_privs($priv='',$table='',$show_privs=false) {
  ...
  if (!zen_not_null($zdb_server)) $zdb_server = zen_spec_read_config_value('DB_SERVER');
  if (!zen_not_null($zdb_user)) $zdb_user     = zen_spec_read_config_value('DB_SERVER_USERNAME');
  if (!zen_not_null($zdb_name)) $zdb_name     = zen_spec_read_config_value('DB_DATABASE');
...
function zen_spec_drop_index_command($param) {
  if (!$checkprivs = zen_spec_check_database_privs('INDEX')) return sprintf(REASON_NO_PRIVILEGES,'INDEX');
...
function zen_spec_create_index_command($param) {
...
function zen_spec_check_alter_command($param) {
  global $db;
  if (!zen_not_null($param)) return "Empty SQL Statement";
  if (!$checkprivs = zen_spec_check_database_privs('ALTER')) return sprintf(REASON_NO_PRIVILEGES,DB_SERVER_USERNAME, DB_SERVER, 'ALTER');
...
function zen_spec_check_config_key($line) {
  global $db;
  $values=array();
  $values=explode("'",$line);
...
function zen_spec_check_product_type_layout_key($line) {
  global $db;
  $values=array();
  $values=explode("'",$line);
  $title = $values[1];
...
function zen_spec_check_cfggroup_key($line) {
  global $db;
  $values=array();
  $values=explode("'",$line);
  $id = $values[1];
...
function zen_spec_write_to_upgrade_exceptions_table($line, $reason, $sql_file) {
  global $db;
  zen_spec_create_exceptions_table();
...
function zen_spec_create_exceptions_table() {
  global $db;
  if (!zen_spec_table_exists(TABLE_UPGRADE_EXCEPTIONS)) {  
...
function zen_spec_check_exceptions($result, $line) {
