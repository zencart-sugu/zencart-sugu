	...
	_getInnerHtml: function() {
		...
		// トップレベルか、そのひとつ下か？
			...
			if ( this.childList || this.data.isLazy ) {
	   			res += cache.tagExpanderTop;
			} else {
	   			res += cache.tagConnectorTop;
			}
...
//	            alert("HIT "+ cn.className);
	...
	getEventTargetType: function(event) {
		...
		if( tcn == cns.title )
			return "title";
		else if( tcn==cns.expander || tcn==cns.expander+"-top")
			return "expander";
		else if( tcn==cns.checkbox )
			return "checkbox";
		else if( tcn==cns.nodeIcon )
			return "icon";
		else if( tcn==cns.empty || tcn==cns.vline || tcn==cns.connector || tcn==cns.connector+"-top")
			return "prefix";
...
DynaTree.prototype = {
	...
	initialize: function(divContainer, options) {
		...
		// Cached tag strings
		this.cache = {
			tagExpanderTop: "<span class='" + options.classNames.expander + "-top'></span>",
			tagConnectorTop: "<span class='" + options.classNames.connector + "-top'></span>",
