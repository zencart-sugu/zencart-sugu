...
?>
<br>
<h1 id="cartDefaultHeading"><?php echo BOX_HEADING_COLORBOX.HEADING_TITLE; ?></h1>
...
<?php echo TEXT_INFORMATION; ?></div>
<?php if (!empty($totalsDisplay)) { ?>
  <div class="cartTotalsDisplay important">
<?php echo $totalsDisplay; ?></div>
<?php } ?>
...
<?php 
echo get_pictgram_accesskey($i).' href="';
echo $product['linkProductsName'].'">'; ?>
<?php echo $product['productsName'] . $product['flagStockCheck']; ?>
<div id="cartImage" class="back">
<?php echo $product['productsImage']; ?></div>
...
<?php
  if ($product['flagShowFixedQuantity']) {
    echo $product['showFixedQuantityAmount'] . $product['flagStockCheck'] . '<br />' . $product['showMinUnits'];
  } else {
    echo $product['quantityField']  . $product['flagStockCheck'] . '<br />' . $product['showMinUnits'];
  }
?>
       </td>
...
?>
           <a href="<?php echo zen_href_link(FILENAME_SHOPPING_CART, 'action=remove_product&product_id=' . $product['id']); ?>"><?php echo TEXT_TRASH_BOX;//echo zen_image($template->get_template_dir(ICON_IMAGE_TRASH, DIR_WS_TEMPLATE, $current_page_base,'images/icons'). '/' . ICON_IMAGE_TRASH, ICON_TRASH_ALT); ?></a>
