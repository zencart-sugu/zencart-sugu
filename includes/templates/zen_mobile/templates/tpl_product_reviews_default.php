...
<?php
        ...
        if (zen_has_product_attributes($review->fields['products_id'] )) {
          ...
          $link= '<a href="' . zen_href_link($_GET['main_page'], zen_get_all_get_params(array('action', 'reviews_id')) . 'action=buy_now') . '">' .BUTTON_IN_CART_ALT.'</a>';
        }
        $the_button = $link;
        $products_link = '';
...
<div id="productReviewsDefaultProductPageLink" class="buttonRow"><?php echo '<a href="' . zen_href_link(zen_get_info_page($_GET['products_id']), zen_get_all_get_params(array('reviews_id'))) . '">' . BUTTON_GOTO_PROD_DETAILS_ALT . '</a>'; ?></div>
</div>
...
<div  id="productReviewsDefaultReviewer" class="bold">
<?php echo sprintf(TEXT_REVIEW_DATE_ADDED, zen_date_short($reviews['dateAdded'])); ?>
<br>
<?php echo sprintf(TEXT_REVIEW_BY, zen_output_string_protected($reviews['customersName'])); ?></div>
<div class="rating">
<?php 
...
echo zen_break_string(zen_output_string_protected(stripslashes($reviews['reviewsText'])), 60, '') . ((strlen($reviews['reviewsText']) >= 30) ? '...' : ''); ?></div>
