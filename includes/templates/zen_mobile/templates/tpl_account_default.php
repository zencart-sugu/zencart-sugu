<?php
/**
 * Page Template
 *
 * Loaded automatically by index.php?main_page=account.<br />
 * Displays previous orders and options to change various Customer Account settings
 *
 * @package templateSystem
 * @copyright Copyright 2003-2005 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: tpl_account_default.php 3105 2006-03-04 18:24:20Z wilt $
 */
echo "<font color=red>".$mobile->view_securepage_notice()."</font>";
?>

<div class="centerColumn" id="accountDefault">
  
<h1 id="accountDefaultHeading"><?php echo BOX_HEADING_COLORBOX . HEADING_TITLE; ?></h1>
<?php if ($messageStack->size('account') > 0) echo $messageStack->output('account'); ?> 

<?php
if (zen_count_customer_orders() > 0) {
?>
    <br>
<?php echo '<a href="' . zen_href_link(FILENAME_ACCOUNT_HISTORY, '', 'SSL') . '">' . OVERVIEW_SHOW_ALL_ORDERS . '</a>'; ?>
    <br>
    <br>
<?php echo BOX_HEADING_COLORBOX.OVERVIEW_PREVIOUS_ORDERS;
  echo "<br>";
    foreach($ordersArray as $orders) {
?>
    <hr size="1" width="95%" align="center" color="<?php echo MOBILE_THEME_COLOR?>">
<?php 
echo "<font size = 2>".BOX_HEADING_CATEGORIES_LIST."</font>";
echo TABLE_HEADING_ORDER_NUMBER;
echo TEXT_NUMBER_SYMBOL . $orders['orders_id'];
echo "<br>";
echo "<font size = 2>".BOX_HEADING_CATEGORIES_LIST."</font>".TABLE_HEADING_DATE;
echo "<br>";
echo zen_date_short($orders['date_purchased']); 
echo "<br>";
echo "<font size = 2>".BOX_HEADING_CATEGORIES_LIST."</font>";
echo TABLE_HEADING_SHIPPED_TO;
echo "<br>";
echo zen_output_string_protected($orders['order_name']) . '<br />' . $orders['order_country']; 
echo "<br>";
echo "<font size = 2>".BOX_HEADING_CATEGORIES_LIST."</font>";
echo TABLE_HEADING_STATUS;
echo ":";
echo $orders['orders_status_name']; 
echo "<br>";
echo "<font size = 2>".BOX_HEADING_CATEGORIES_LIST."</font>";
echo TABLE_HEADING_TOTAL;
echo ":";
echo $orders['order_total']; 
echo "<br>";
echo '<a href="' . zen_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . $orders['orders_id'], 'SSL') . '"> ' . zen_image_button(BUTTON_IMAGE_VIEW_SMALL, BUTTON_VIEW_SMALL_ALT) . '</a>'; 

echo "<br>";
  
  }//foreach


}//if
?>
<hr size="1" width="95%" align="center" color="<?php echo MOBILE_THEME_COLOR?>">
<br class="clearBoth" />
<div id="accountLinksWrapper" class="back">
<h2><?php echo BOX_HEADING_COLORBOX . MY_ACCOUNT_TITLE; ?></h2>
<ul id="myAccountGen" class="list">
<li><?php echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_EDIT, '', 'SSL') . '">' . MY_ACCOUNT_INFORMATION . '</a>'; ?></li>
<li><?php echo ' <a href="' . zen_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL') . '">' . MY_ACCOUNT_ADDRESS_BOOK . '</a>'; ?></li>
<li><?php echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_PASSWORD, '', 'SSL') . '">' . MY_ACCOUNT_PASSWORD . '</a>'; ?></li>
</ul>


<?php
  if (SHOW_NEWSLETTER_UNSUBSCRIBE_LINK !='false' or CUSTOMERS_PRODUCTS_NOTIFICATION_STATUS !='0') {
?>
<h2><?php echo BOX_HEADING_COLORBOX.EMAIL_NOTIFICATIONS_TITLE; ?></h2>
<?php
  if (SHOW_NEWSLETTER_UNSUBSCRIBE_LINK=='true') {
?>
<ul id="myAccountNotify" class="list">
<li><?php echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_NEWSLETTERS, '', 'SSL') . '">' . EMAIL_NOTIFICATIONS_NEWSLETTERS . '</a>'; ?></li>
<?php } //endif newsletter unsubscribe ?>
<?php
  if (CUSTOMERS_PRODUCTS_NOTIFICATION_STATUS == '1') {
?>
<li><?php echo ' <a href="' . zen_href_link(FILENAME_ACCOUNT_NOTIFICATIONS, '', 'SSL') . '">' . EMAIL_NOTIFICATIONS_PRODUCTS . '</a>'; ?></li>
</ul>

<?php } //endif product notification ?>
  
<?php } // endif don't show unsubscribe or notification ?>
<h2>
<?php
echo  BOX_HEADING_COLORBOX.TEXT_EASY_LOGIN_CONFIG
?>
</h2>
<li>
<?php
echo ' <a href="' . zen_href_link(FILENAME_EASY_LOGIN_CONFIG, '', 'SSL') . '">' . TEXT_EASY_LOGIN_CONFIG . '</a>'; ?>
</li>
</div>
<?php
// only show when there is a GV balance
  if ($customer_has_gv_balance ) {
?>
<div id="sendSpendWrapper">
<?php require($template->get_template_dir('tpl_modules_send_or_spend.php',DIR_WS_TEMPLATE, $current_page_base,'templates'). '/tpl_modules_send_or_spend.php'); ?>
</div>
<?php
  }
?>
<br class="clearBoth" />
</div>
