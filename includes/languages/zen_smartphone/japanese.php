...
//$templates = require($template->get_template_dir('/tpl_modules_listing_display_order.php',DIR_WS_TEMPLATE, $current_page_base,'templates'). '/tpl_modules_listing_display_order.php');

//共有
if(MODULE_ADDON_MODULES_REFER_RESOURCE_SERVER_STATUS == 'true'){
  global $request_type;
  if ($request_type == "NONSSL")
    $prefix_image_host = MODULE_ADDON_MODULES_REFER_RESOURCE_SERVER_HOST_URL . '/';
  else
...
// -> zen_smartphone: Greeting部分は新しくdefineした方を使う
//if (STORE_STATUS == '0') {
//  define('TEXT_GREETING_GUEST', '');
//} else {
//  define('TEXT_GREETING_GUEST', 'こんにちは、<span>%s</span>さん。');
//}
define('TEXT_GREETING_FOR_SMARTPHONE_GUEST', '<a class="whiteButton" href="%s">ログイン</a>');
define('TEXT_GREETING_FOR_SMARTPHONE_PERSONAL', '<a class="whiteButton" href="%s">%sさんのマイページ</a>
<a class="grayButton" href="%s">ログアウト</a>');
// <- zen_smartphone: Greeting部分は新しくdefineした方を使う

define('TEXT_GREETING_PERSONAL', '%s');
/*
define('HEADER_TEXT_FOR_VISITOR', '
<p id="greeting">こんにちは、<span>'.zen_customer_greeting().'</span>さん</p>
');
*/
define('HEADER_TEXT_FOR_VISITOR', '<p id="greeting">こんにちは。<a href="'.zen_href_link(FILENAME_ADDON, '&module=visitors/visitor_to_account', 'SSL').'">入力情報を引き継いでアカウント登録</a>もできます。</p>');

define('HEADER_TEXT_FOR_ACCOUNT', '
<p id="greeting"><strong>こんにちは。&nbsp;<a href="'.zen_href_link(FILENAME_ACCOUNT, '', 'SSL').'" title="'.zen_customer_greeting().'さんのページ">'.zen_customer_greeting().'さんのページ</a>へ</strong> | <a href="'. zen_href_link(FILENAME_LOGOFF, '', 'NONSSL').'" title="ログアウト">ログアウト</a></p>
');
...
<dd>
<ul>
<li class="font-small"><a href="javascript:void(0)" rel="styles3" class="styleswitch" id="id-font-small">フォントサイズを小さく</a></li>
<li class="font-nomal"><a href="javascript:void(0)" rel="styles1" class="styleswitch" id="id-font-normal">フォントサイズを元に戻す</a></li>
<li class="font-large"><a href="javascript:void(0)" rel="styles2" class="styleswitch" id="id-font-large">フォントサイズを大きく</a></li>
</ul>
</dd>
</dl>
<!-- /font-size -->
...
define('HEADLINE_INFORMATION', '<h2>お知らせ</h2>');
define('TEXT_PRICE_KN', '販売価格：');
define('TEXT_PRICE_TAX', '<span class="tax">(税込)</span>');

define('PAGETOP', 'このページの先頭へ戻る');

//商品一覧
define('TEXT_PRICE_EN', '円');
define('DOB_FORMAT_STRING_SAMPLE', '<span>例：2009/01/01　〜　2009/06/30</span>');
define('KEYWORD_FORMAT_STRING', 'キーワードを入力してください');

//商品詳細
define('TEXT_PRODUCT_PRICE', '価格');
define('TEXT_PRODUCT_SECURITY', '延長保障');
define('PRODUCTS_ORDER_QTY', '数量');
define('HEADELINE_PRODUCT_DESCRIPTION', '<h2>商品の説明</h2>');


//会員登録
define('TABLE_HEADING_NAME', '名前');
define('ENTRY_NAME', '姓名（漢字）');
define('ENTRY_KANA', '姓名（ひらがな）');

define('ENTRY_SAMPLE_01', '半角英数字　例：example@sample.co.jp');
define('ENTRY_SAMPLE_02', '半角英数字　5文字以上');
define('ENTRY_SAMPLE_03', '確認のためもう一度パスワードを入力してください');
define('ENTRY_SAMPLE_04', ''); // 例：日本');
define('ENTRY_SAMPLE_05', ''); // 例：太郎');
define('ENTRY_SAMPLE_06', ''); // 例：にほん');
define('ENTRY_SAMPLE_07', ''); // 例：たろう');
define('ENTRY_SAMPLE_08', '半角数字・ハイフン(-)あり　例： 123-4567');
define('ENTRY_SAMPLE_09', '例： 渋谷区');
define('ENTRY_SAMPLE_10', '例： ○○町1-2-3');
define('ENTRY_SAMPLE_11', '例： ○○マンション○○○号室');
define('ENTRY_SAMPLE_12', '半角数字・ハイフン(-)あり　例： 03-1234-5678');
define('ENTRY_SAMPLE_13', '例： 1970/05/21');
define('ENTRY_SAMPLE_00', '<a href="http://www.post.japanpost.jp/zipcode/" target="_blank">郵便番号を調べる</a>');


//注文手続き
define('HEADING_COMMENTS', '備考欄');
define('TABLE_HEADING_PRICE','単価');
define('ORDERTOTALS_SHIPPUING','配送料');
...
// if mbstring is not load, use mb-emulator 
  ...
  define('DATE_FORMAT_SHORT', '%Y年%m月%d日');  // this is used for strftime()
  define('DATE_FORMAT_LONG', '%Y年%m月%d日&nbsp;%a曜日'); // this is used for strftime()
  define('DATE_FORMAT', 'Y年m月d日'); // this is used for date()
...
//text for sidebox heading links
  define('BOX_HEADING_LINKS', '&nbsp;&nbsp;[一覧]');

// categories box text in sideboxes/categories.php
  define('BOX_HEADING_CATEGORIES', '商品カテゴリ');
...
// whats_new box text in sideboxes/whats_new.php
  ...
  define('CATEGORIES_BOX_HEADING_PRODUCTS_ALL', '商品一覧');

// quick_find box text in sideboxes/quick_find.php
  define('BOX_HEADING_SEARCH', '商品を検索する');
...
// reviews box text in sideboxes/reviews.php
  ...
  define('BOX_REVIEWS_TEXT_OF_5_STARS', '平均評価：5点満点中%s点 ');
...
// best_sellers box text in sideboxes/best_sellers.php
  define('BOX_HEADING_BESTSELLERS', '人気商品ランキング');
...
// javascript messages
  ...
  define('ENTRY_GENDER_TEXT', '必須');
  define('ENTRY_FIRST_NAME', '姓(漢字)');
  define('ENTRY_FIRST_NAME_ERROR', '姓は最低' . ENTRY_FIRST_NAME_MIN_LENGTH . '文字以上入力してください');
  define('ENTRY_FIRST_NAME_TEXT', '必須');
  define('ENTRY_LAST_NAME', '名(漢字)');
  define('ENTRY_LAST_NAME_ERROR', '名は最低' . ENTRY_LAST_NAME_MIN_LENGTH . '文字以上入力してください');
  define('ENTRY_LAST_NAME_TEXT', '必須');
  // ->furikana
  define('ENTRY_FIRST_NAME_KANA', '姓(ひらがな)');
  define('ENTRY_FIRST_NAME_KANA_ERROR', '姓ひらがなは最低' . ENTRY_FIRST_NAME_MIN_LENGTH . '文字以上入力してください');
  define('ENTRY_FIRST_NAME_KANA_TEXT', '必須');
  define('ENTRY_LAST_NAME_KANA', '名(ひらがな)');
  define('ENTRY_LAST_NAME_KANA_ERROR', '名ひらがなは最低' . ENTRY_LAST_NAME_MIN_LENGTH . '文字以上入力してください');
  define('ENTRY_LAST_NAME_KANA_TEXT', '必須');
  // <-furikana
  define('ENTRY_DATE_OF_BIRTH', '誕生日');
  define('ENTRY_DATE_OF_BIRTH_ERROR', '誕生日は[1970/05/21](西暦/月/日)の書式で入力してください');
  define('ENTRY_DATE_OF_BIRTH_TEXT', '必須 (記入例 1970/05/21)');
  define('ENTRY_EMAIL_ADDRESS', 'メールアドレス');
  define('ENTRY_EMAIL_ADDRESS_ERROR', 'メールアドレスは半角で' . ENTRY_EMAIL_ADDRESS_MIN_LENGTH . '文字以上入力してください');
  ...
  define('ENTRY_EMAIL_ADDRESS_TEXT', '必須');
  ...
  define('ENTRY_STREET_ADDRESS_ERROR', '町名・番地は最低' . ENTRY_STREET_ADDRESS_MIN_LENGTH . '文字以上入力してください');
  define('ENTRY_STREET_ADDRESS_TEXT', '必須');
  define('ENTRY_SUBURB', '建物名など');
  define('ENTRY_SUBURB_ERROR', '');
  define('ENTRY_SUBURB_TEXT', '');
  define('ENTRY_POST_CODE', '郵便番号');
  define('ENTRY_POST_CODE_ERROR', '郵便番号は最低' . ENTRY_POSTCODE_MIN_LENGTH . '文字以上入力してください');
  define('ENTRY_POST_CODE_TEXT', '必須');
  ...
  define('ENTRY_PASSWORD_TEXT', '必須 (半角英数字で最低' . ENTRY_PASSWORD_MIN_LENGTH . '文字以上)');
  define('ENTRY_PASSWORD_CONFIRMATION', 'パスワードの確認');
  define('ENTRY_PASSWORD_CONFIRMATION_TEXT', '必須');
  define('ENTRY_PASSWORD_CURRENT', '現在のパスワード');
  define('ENTRY_PASSWORD_CURRENT_TEXT', '必須');
  ...
  define('ENTRY_PASSWORD_NEW_TEXT', '必須');
  ...
  define('FORM_REQUIRED_INFORMATION', '必須項目はすべて入力してください');
  define('ENTRY_REQUIRED_SYMBOL', '必須');
...
define('TEXT_DISPLAY_NUMBER_OF_PRODUCTS', '<strong>%d件目</strong>から<strong>%d件目</strong>を表示中&nbsp;(商品の数:&nbsp;<strong>全%d商品</strong>）');
define('TEXT_DISPLAY_NUMBER_OF_ORDERS', '<strong>%d件目</strong>から<strong>%d件目</strong>を表示中&nbsp;(ご注文数: <strong>全%d商品</strong>)');
define('TEXT_DISPLAY_NUMBER_OF_REVIEWS', '<strong>%d件目</strong>から<strong>%d件目</strong>を表示中&nbsp;(レビュー数: <strong>全%d件</strong>)');
define('TEXT_DISPLAY_NUMBER_OF_PRODUCTS_NEW', '<strong>%d件目</strong>から<strong>%d件目</strong>を表示中&nbsp;(新着商品の数:&nbsp;<strong>全%d商品</strong>)');
define('TEXT_DISPLAY_NUMBER_OF_SPECIALS', '<strong>%d件目</strong>から<strong>%d件目</strong>&nbsp;を表示中&nbsp;(特価商品の数:&nbsp;<strong>全%d商品</strong>)');
define('TEXT_DISPLAY_NUMBER_OF_PRODUCTS_FEATURED_PRODUCTS', '<strong>%d件目</strong>から<strong>%d件目</strong>を表示中&nbsp;(おすすめ商品の数:&nbsp;<strong>全%d商品</strong>)');
define('TEXT_DISPLAY_NUMBER_OF_PRODUCTS_ALL', '<strong>%d件目</strong>から<strong>%d件目</strong>を表示中 (全商品の数:<strong>%d商品</strong>)');

define('PREVNEXT_TITLE_FIRST_PAGE', '最初');
define('PREVNEXT_TITLE_PREVIOUS_PAGE', '前へ');
define('PREVNEXT_TITLE_NEXT_PAGE', '次へ');
define('PREVNEXT_TITLE_LAST_PAGE', '最後');
...
define('TEXT_NO_REVIEWS', '商品のレビューはまだありません。');
 ...
 // define('TEXT_REQUIRED', '<span class="errorText">必須</span>');
	define('TEXT_REQUIRED', '<img src="'.$template->get_template_dir('icon_required.gif',DIR_WS_TEMPLATE, $current_page_base,'images/').'/icon_required.gif" alt="必須" />');
	define('TEXT_REQUIRED2', '<img src="'.$template->get_template_dir('icon_required2.gif',DIR_WS_TEMPLATE, $current_page_base,'images/').'/icon_required.gif" alt="必須" />');
...
// cart contents
  ...
  define('PRODUCTS_ORDER_QTY_TEXT','個');
...
// End iii added
  ...
  define('PRODUCTS_PRICE_IS_CALL_FOR_PRICE_TEXT','価格はお問い合わせください');
...
// shopping cart errors
  ...
  define('TABLE_HEADING_NEW_PRODUCTS', '新商品');
...
// Shipping Estimator
  ...
  define('CART_SHIPPING_METHOD_RECALCULATE_TXT','数量を変更した場合は「再計算」ボタンを押してください。');
...
// multiple product add to cart
  define('TEXT_PRODUCT_LISTING_MULTIPLE_ADD_TO_CART', '追加数量: ');
...
// sort order titles for dropdowns
  ...
  define('TEXT_INFO_SORT_BY_PRODUCTS_SORT_ORDER', '既定の表示');
	...
	define('TEXT_INFO_SORT_BY_PRODUCTS_MANUFACTURER', 'メーカー名');
  define('TEXT_INFO_SORT_BY_PRODUCTS_MANUFACTURER_DESC', 'メーカー - 降順');
...
// table headings for cart display and upcoming products
  ...
  define('TABLE_HEADING_DELETE', '削除');

// create account - login shared
  define('TABLE_HEADING_PRIVACY_CONDITIONS', '個人情報保護方針');
  define('TEXT_PRIVACY_CONDITIONS_DESCRIPTION', '弊社の個人情報保護方針に同意される場合はチェックボックスをクリックしてください。<a href="'.zen_href_link(FILENAME_ADDON, '&module=mt_pages&page=privacy_policy', 'NONSSL').'">個人情報保護方針はこちら</a>。');
  ...
  define('TABLE_HEADING_DATE_OF_BIRTH', '性別・誕生日');
  define('TABLE_HEADING_LOGIN_DETAILS', 'ログイン情報');
  define('LOGIN_DETAILS_TEXT', 'ログインするときに使うメールアドレスとパスワードを入力してください。');
