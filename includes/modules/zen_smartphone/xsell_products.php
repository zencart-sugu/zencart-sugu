...
".// -> zen_smartphone: ページングの概念を追加
//                                 order by xp.sort_order asc limit " . MAX_DISPLAY_XSELL);
                                 "order by xp.sort_order asc limit " . MAX_DISPLAY_XSELL ." ".
                                 "offset ". (isset($_GET['page']) ? SHOW_PRODUCT_INFO_COLUMNS_XSELL_PRODUCTS * (int)($_GET['page'] - 1) : 0));
// <- zen_smartphone: ページングの概念を追加
...
// -> zen_smartphone: 全件数を取得する
  $xsell_query = $db->Execute("select distinct count(p.products_id) as max_count
                                 from " . TABLE_PRODUCTS_XSELL . " xp, " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd
                                 where xp.products_id = '" . $_GET['products_id'] . "'
                                  and xp.xsell_id = p.products_id
                                  and p.products_id = pd.products_id
                                  and pd.language_id = '" . $_SESSION['languages_id'] . "'
                                  and p.products_status = 1");
  $max_products_xsell = $xsell_query->fields['max_count'];
// <- zen_smartphone: ページングの概念を追加
