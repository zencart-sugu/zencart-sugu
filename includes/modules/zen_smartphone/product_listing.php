 ...
 * UPDATED TO WORK WITH COLUMNAR PRODUCT LISTING For Zen Cart v1.3.0.2 - 07/04/2006
 */
...
// -> zen_smartphone: 先頭の1行はコメントアウトしたので$rows++は下でやる
//      $rows++;
// <- zen_smartphone: 先頭の1行はコメントアウトしたので$rows++は下でやる

      if ((($rows-$extra_row)/2) == floor(($rows-$extra_row)/2)) {
        $list_box_contents[$rows] = array('params' => 'class="productListing-even arrow"');
      } else {
        $list_box_contents[$rows] = array('params' => 'class="productListing-odd arrow"');
      }
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        case 'PRODUCT_LIST_MODEL':
          ...
          break;
        case 'PRODUCT_LIST_NAME':
// -> zen_smartphone: PRODUCT_LIST_PRICEに移動
//          $lc_align = '';
...
// <- zen_smartphone: PRODUCT_LIST_PRICEに移動
        ...
        case 'PRODUCT_LIST_MANUFACTURER':
          ...
          break;
        case 'PRODUCT_LIST_PRICE':
// -> zen_smartphone: PRODUCT_LIST_NAMEから。
          $lc_align = '';
          if (isset($_GET['manufacturers_id'])) {
            $lc_text = '<h3 class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'manufacturers_id=' . $_GET['manufacturers_id'] . '&products_id=' . $listing->fields['products_id']) . '">' . $listing->fields['products_name'] . '</a></h3><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>' ;
          } else {
...
// -> zen_smartphone: <br />がジャマ。必要なら出すように変更
//          $lc_text .= '<br />' . zen_get_buy_now_button($listing->fields['products_id'], $the_button, $products_link) . '<br />' . zen_get_products_quantity_min_units_display($listing->fields['products_id']);
//          $lc_text .= '<br />' . (zen_get_show_product_switch($listing->fields['products_id'], 'ALWAYS_FREE_SHIPPING_IMAGE_SWITCH') ? (zen_get_product_is_always_free_shipping($listing->fields['products_id']) ? TEXT_PRODUCT_FREE_SHIPPING_ICON . '<br />' : '') : '');
          $add_lc_text_list = array();
          $add_lc_text_list[] = zen_get_buy_now_button($listing->fields['products_id'], $the_button, $products_link);
          $add_lc_text_list[] = zen_get_products_quantity_min_units_display($listing->fields['products_id']);
          $add_lc_text_list[] = (zen_get_show_product_switch($listing->fields['products_id'], 'ALWAYS_FREE_SHIPPING_IMAGE_SWITCH') ? (zen_get_product_is_always_free_shipping($listing->fields['products_id']) ? TEXT_PRODUCT_FREE_SHIPPING_ICON . '<br />' : '') : '');
          foreach ($add_lc_text_list as $add_lc_text) {
            if ($add_lc_text == '') continue;
            $lc_text .= '<br />'. $add_lc_text;
          }
// <- zen_smartphone: <br />がジャマ。必要なら出すように変更
...
// <- zen_smartphone: $lc_textがNULLなら(主にPRODUCT_LIST_NAMEの場合)、追加しない
    ...
    $listing->MoveNext();
// -> zen_smartphone: 先頭の1行はコメントアウトしたので$rows++はココでやる
    $rows++;
// <- zen_smartphone: 先頭の1行はコメントアウトしたので$rows++はココでやる
  }
