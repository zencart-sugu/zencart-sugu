 ...
 */
$zco_notifier->notify('NOTIFY_CREATE_ACCOUNT_VARS');
...
# if (ACCOUNT_DOB == 'true') $dob = (empty($_POST['dob']) ? zen_db_prepare_input('0001-01-01 00:00:00') : zen_db_prepare_input($_POST['dob']));
  ...
  if (strlen($password) < ENTRY_PASSWORD_MIN_LENGTH) {
   ...
   $mobile_sn = $mobile->getSerialNumber();
   if($mobile_sn){  
       $sn_exist = $db->Execute("SELECT customers_id FROM ".TABLE_CUSTOMERS." WHERE customers_mobile_serial_number = '".$mobile_sn."'");
       if($sn_exist->fields['customers_id']){
           $error = true;
           $messageStack->add('create_account',MOBILE_SERIAL_DUPLICATION_ERROR);
       }   
   }
  if ($error == true) {
    // hook notifier class
    ...
    // <-furikana
    if($mobile_sn) $sql_data_array['customers_mobile_serial_number'] = $mobile_sn;
